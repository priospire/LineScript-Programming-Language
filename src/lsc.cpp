...
private:
  ...
  void emitBuiltins() {
    ...
    o_ << "  g->fixed_dt = dt;\n";
    o_ << "}\n";
    o_ << "static inline void game_set_fullscreen(int64_t id, ls_bool enabled) {\n";
    o_ << "  ls_game *g = ls_get_game(id);\n";
    o_ << "  if (!g) return;\n";
    o_ << "#if defined(_WIN32)\n";
    o_ << "  if (g->headless || !g->hwnd) return;\n";
    o_ << "  const ls_bool want = enabled ? 1 : 0;\n";
    o_ << "  if (g->fullscreen == want) return;\n";
    o_ << "  if (want) {\n";
    o_ << "    MONITORINFO mi;\n";
...
static constexpr int kLineScriptVerHalfSteps = 0;  // Velocity release: displayed 1.5.0
