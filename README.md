...
- preset libraries: `docs/LIBRARIES.md`
...
Compile with a preset library:

```powershell
.\linescript.cmd .\libraries\math\math_extras.lsc .\main.lsc -O4 --cc clang
```

Compile with the game-oriented helper packs:
...
### Method 0: setup wizard

The Windows dist bundle includes `LineScriptSetup.exe`.

1. Extract `LineScript-win64-YYYYMMDD.zip`.
2. Run `LineScriptSetup.exe`.
3. Pick an install folder.
4. Choose optional preset library packs.
   - Core packs: math, text, collections, HTTP.
   - Game engine packs: window lifecycle, UI helpers, 2D helpers, 3D projection helpers, and physics helpers.
   - Tooling: VSCode extension files and optional test suites.
5. Click `Install`.

The installer copies LineScript into the selected folder and writes dependency status to `dependencies\DEPENDENCY_STATUS.txt`.

### Method 0.5: install directly from GitHub

Use the built-in installer when installing from GitHub:

1. Open https://github.com/priospire/LineScript-Programming-Language.
2. Download the repository ZIP or clone the repository.
3. Open the included `dist/LineScript-win64-YYYYMMDD` folder.
4. Run `LineScriptSetup.exe`.
5. Choose your install folder and library packs.

Do not manually hunt for compiler pieces when installing from GitHub. The setup wizard copies the compiler, scripts, docs, examples, editor support, and selected libraries into one usable folder.
