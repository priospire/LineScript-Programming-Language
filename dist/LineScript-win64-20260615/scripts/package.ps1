...
$installerSrc = Join-Path $root "installer\\LineScriptSetup.cpp"
$installerExe = Join-Path $root "installer\\LineScriptSetup.exe"
...
if (-not $SkipBuild) {
  ...
  if (Test-Path $installerSrc) {
    & powershell -ExecutionPolicy Bypass -File (Join-Path $root "scripts\\build_installer.ps1") -Compiler $Compiler
...
if (Test-Path $installerExe) {
  Copy-Item $installerExe (Join-Path $bundleDir "LineScriptSetup.exe")
}
...
$librariesOut = Join-Path $bundleDir "libraries"
...
New-Item -ItemType Directory -Path $librariesOut | Out-Null
...
foreach ($scriptName in @("build_installer.ps1", "install_vscode_extension.ps1", "install_vscode_extension.sh", "package.ps1", "package.sh")) {
...
if (Test-Path (Join-Path $root "libraries")) {
  Copy-Item (Join-Path $root "libraries\\*") $librariesOut -Recurse
}
