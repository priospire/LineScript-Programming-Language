...
const defaultSettings = {
  ...
  checkOnType: true,
  checkOnSave: true,
  validationDelayMinMs: 500,
  validationDelayMaxMs: 3500,
  checkTimeoutMs: 8000,
  extraCheckArgs: [],
  hintsEnabled: true,
...
const builtinSignatures = [
  ...
  "gfx_draw_bitmap(canvas: i64, bitmap: i64, x: i64, y: i64) -> void",
  ...
  "game_draw_bitmap(game: i64, bitmap: i64, x: i64, y: i64) -> void",
...
function scheduleValidate(document, forceCompiler) {
  ...
  getDocumentSettings(document.uri)
    .then((settings) => {
      ...
      const minDelay = clampDelay(merged.validationDelayMinMs, 500);
      const maxDelay = clampDelay(merged.validationDelayMaxMs, 3500);
