...
If you install from GitHub, download or clone the repository, open the included `dist\LineScript-win64-<date>` folder, and run `LineScriptSetup.exe`. The installer is the recommended GitHub install path because it copies the compiler, scripts, docs, examples, editor support, and selected libraries into one folder.

Windows setup wizard:

1. Extract `dist\LineScript-win64-<date>.zip`.
2. Run `LineScriptSetup.exe`.
3. Choose the folder where LineScript should be installed.
4. Select any preset libraries you want installed.
