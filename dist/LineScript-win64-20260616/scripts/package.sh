...
[[ -f "$root/installer/LineScriptSetup.exe" ]] && cp "$root/installer/LineScriptSetup.exe" "$bundle_dir/LineScriptSetup.exe"
...
for script_name in build_installer.ps1 install_vscode_extension.ps1 install_vscode_extension.sh package.ps1 package.sh; do
...
if [[ -d "$root/libraries" ]]; then
  cp -r "$root/libraries/." "$bundle_dir/libraries/"
fi
